...
describe("Backend", () => {
    ...
    beforeEach(() => {
        subject = new BackendPolling("baseUrl", MockStore());
    ...
    xdescribe("buildDetails connection", () => {
    ...
    describe("build summaries connection", () => {
        it("should request summaries endpoint", () => {
            mockFetchResponse({summaries: []});

            subject.requestSummaries(dispatchMock);

            expect(fetch).toBeCalledWith("http://baseUrl/lambdaui/api/builds");
        });

        xit("should dispatch incoming message to the store", () => {
            ...
            expect(dispatchMock).toBeCalledWith({
                type: "addBuildSummaries",
                summaries: {buildId: 1, status: "finished"}
            });
        });

        xit("should dispatch connection state of summaries connection", () => {
            subject.requestSummaries(dispatchMock);
