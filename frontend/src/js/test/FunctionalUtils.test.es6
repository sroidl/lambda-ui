...
import * as subject from "../main/FunctionalUtils.es6";
...
describe("FunctionalUtils", () => {
    ...
    describe("mapTree", () => {
        it("should return mapped tree", () => {
            ...
            const mp = step => {
                return {id: step.id, steps: step.steps};
            };
    ...
    describe("getFlatTree", () => {
        it("should return flat tree", () => {
            const input = {
                buildId: 1, subElements: [{name: "element1", subElements: [{name: "element1-1"}]}
                    , {name: "element2", subElements: [{name: "element2-1"}]}]
            };
            const expected = [{name: "element1-1"}, {name: "element1", subElements: [{name: "element1-1"}]},
                {name: "element2-1"}, {name: "element2", subElements: [{name: "element2-1"}]}];
    ...
    describe("getFlatSteps", () => {
        ...
        it("should return flat array with steps", () => {
            ...
            const expected = [{stepId: "1-1", state: "failure", parentId: "1", steps: []},
                {
                    stepId: "1",
                    state: "failure",
                    parentId: "root",
                    steps: [{stepId: "1-1", state: "failure", parentId: "1", steps: []}]
                },
                {stepId: "2-1", state: "failure", parentId: "2", steps: []},
                {
                    ...
                    state: "failure",
                    parentId: "root",
                    steps: [{stepId: "2-1", state: "failure", parentId: "2", steps: []}]
                }];
            expect(subject.getFlatSteps(state, 1)).toEqual(expected);
        });
    });
